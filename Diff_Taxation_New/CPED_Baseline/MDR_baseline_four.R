...
amr <- function(t, y, parms) {
  with(as.list(c(y, parms)), {
    ...
    sigma_use1 <- sigma_mat[1,1]; sigma_use2 <- sigma_mat[2,1];  sigma_use3 <- sigma_mat[3,1]; sigma_use4 <- sigma_mat[4,1]
    
    #For One Round
    if(t > t_n & int_round > 0) { 
      sigma_use1 <- sigma_mat[1,2]; sigma_use2 <- sigma_mat[2,2];  sigma_use3 <- sigma_mat[3,2]; sigma_use4 <- sigma_mat[4,2]
    }
    
    if(t > (t_n + time_between) & int_round > 1) { 
      sigma_use1 <- sigma_mat[1,3]; sigma_use2 <- sigma_mat[2,3];  sigma_use3 <- sigma_mat[3,3]; sigma_use4 <- sigma_mat[4,3]
    }
    
    if(t > (t_n + time_between*2) & int_round > 2) { 
      sigma_use1 <- sigma_mat[1,4]; sigma_use2 <- sigma_mat[2,4];  sigma_use3 <- sigma_mat[3,4]; sigma_use4 <- sigma_mat[4,4]
    }
    
    if(t > (t_n + time_between*3) & int_round > 3) {
      sigma_use1 <- sigma_mat[1,5]; sigma_use2 <- sigma_mat[2,5];  sigma_use3 <- sigma_mat[3,5]; sigma_use4 <- sigma_mat[4,5]
    }
    
    if(t > (t_n + time_between*4) & int_round > 4) { 
      sigma_use1 <- sigma_mat[1,6]; sigma_use2 <- sigma_mat[2,6];  sigma_use3 <- sigma_mat[3,6]; sigma_use4 <- sigma_mat[4,6]
    }
    
    if(t > (t_n + time_between*5) & int_round > 5) {
      sigma_use1 <- sigma_mat[1,7]; sigma_use2 <- sigma_mat[2,7];  sigma_use3 <- sigma_mat[3,7]; sigma_use4 <- sigma_mat[4,7]
    }
    ...
    dR1 = - lambda*R1 + beta*X*R1*c1 - r_t*(1-rho)*(sigma_use2 + sigma_use3 + sigma_use4)*R1 - r_r*sigma_use1*R1 - eta_rr*R1*rho*sigma_use2 -
    ...
    dR2 = - lambda*R2 + beta*X*R2*c2 - r_t*(1-rho)*(sigma_use1 + sigma_use3 + sigma_use4)*R2 - r_r*sigma_use2*R2 - eta_rr*R2*rho*sigma_use1 -
    ...
    dR3 = - lambda*R3 + beta*X*R3*c3 - r_t*(1-rho)*(sigma_use1 + sigma_use2 + sigma_use4)*R3 - r_r*sigma_use3*R3 - eta_rr*R3*rho*sigma_use1 -
    ...
    dR4 = - lambda*R4 + beta*X*R4*c4 - r_t*(1-rho)*(sigma_use1 + sigma_use2 + sigma_use3)*R4 - r_r*sigma_use4*R4 - eta_rr*R4*rho*(sigma_use1 + sigma_use2 + sigma_use3) - 
      eta_rw*R4*(1 - (sigma_use1 + sigma_use2 + sigma_use3 + sigma_use4)) + eta_wr*rho*Wt*sigma_use4
    
    
    dR12 = - lambda*R12 + beta*X*R12*c12 - r_t*(1-rho)*(sigma_use3 + sigma_use4)*R12 - r_rr*(sigma_use1 + sigma_use2)*R12 - eta_rrr*R12*rho*(sigma_use3) - eta_rrr*R12*rho*sigma_use4 -
      eta_rw*R12*(1 - (sigma_use1 + sigma_use2 + sigma_use3 + sigma_use4)) + eta_rr*R1*rho*sigma_use2 + eta_rr*R2*rho*sigma_use1 
    
    dR13 = - lambda*R13 + beta*X*R13*c13 - r_t*(1-rho)*(sigma_use2 + sigma_use4)*R13 - r_rr*(sigma_use1 + sigma_use3)*R13 - eta_rrr*R13*rho*(sigma_use2) - eta_rrr*R13*rho*sigma_use4 -
      eta_rw*R13*(1 - (sigma_use1 + sigma_use2 + sigma_use3 + sigma_use4)) + eta_rr*R1*rho*sigma_use3 + eta_rr*R3*rho*sigma_use1 
    
    dR14 = - lambda*R14 + beta*X*R14*c14 - r_t*(1-rho)*(sigma_use2 + sigma_use3)*R14 - r_rr*(sigma_use1 + sigma_use4)*R14 - eta_rrr*R14*rho*(sigma_use2) - eta_rrr*R14*rho*sigma_use3 -
      eta_rw*R14*(1 - (sigma_use1 + sigma_use2 + sigma_use3 + sigma_use4)) + eta_rr*R1*rho*sigma_use4 + eta_rr*R4*rho*sigma_use1 
    
    dR23 = - lambda*R23 + beta*X*R23*c23 - r_t*(1-rho)*(sigma_use1 + sigma_use4)*R23 - r_rr*(sigma_use2 + sigma_use3)*R23 - eta_rrr*R23*rho*(sigma_use1) - eta_rrr*R23*rho*sigma_use4 -
      eta_rw*R23*(1 - (sigma_use1 + sigma_use2 + sigma_use3 + sigma_use4)) + eta_rr*R2*rho*sigma_use3 + eta_rr*R3*rho*sigma_use2
    
    dR24 = - lambda*R24 + beta*X*R24*c24 - r_t*(1-rho)*(sigma_use1 + sigma_use3)*R24 - r_rr*(sigma_use2 + sigma_use4)*R24 - eta_rrr*R24*rho*(sigma_use1) - eta_rrr*R24*rho*sigma_use3 -
      eta_rw*R24*(1 - (sigma_use1 + sigma_use2 + sigma_use3 + sigma_use4)) + eta_rr*R2*rho*sigma_use4 + eta_rr*R4*rho*sigma_use2 
    
    dR34 = - lambda*R34 + beta*X*R34*c34 - r_t*(1-rho)*(sigma_use1 + sigma_use2)*R34 - r_rr*(sigma_use3 + sigma_use4)*R34 - eta_rrr*R34*rho*(sigma_use1) - eta_rrr*R34*rho*sigma_use2 -
      eta_rw*R34*(1 - (sigma_use1 + sigma_use2 + sigma_use3 + sigma_use4)) + eta_rr*R3*rho*sigma_use4 + eta_rr*R4*rho*sigma_use3 
    
    
    dR123 = - lambda*R123 + beta*X*R123*c123 - r_t*(1-rho)*R123*(sigma_use4) - r_rrr*(sigma_use1 + sigma_use2 + sigma_use3)*R123 - 
    ...
    dR124 = - lambda*R124 + beta*X*R124*c124 - r_t*(1-rho)*R124*(sigma_use3) - r_rrr*(sigma_use1 + sigma_use2 + sigma_use4)*R124 - 
    ...
    dR134 = - lambda*R134 + beta*X*R134*c134 - r_t*(1-rho)*R134*(sigma_use2) - r_rrr*(sigma_use1 + sigma_use3 + sigma_use4)*R134 - 
    ...
    dR234 = - lambda*R234 + beta*X*R234*c234 - r_t*(1-rho)*R234*(sigma_use1) - r_rrr*(sigma_use2 + sigma_use3 + sigma_use4)*R234 - 
    ...
    dR1234 = - lambda*R1234 + beta*X*R1234*c1234 - r_rrrr*(sigma_use1 + sigma_use2 + sigma_use3 + sigma_use4)*R1234 - eta_rw*R1234*(1 - (sigma_use1 + sigma_use2 + sigma_use3 + sigma_use4)) + 
...
ode_wrapper <- function(times, y, parms, func) {
  ...
  if(parms[["int_round"]] > 0 ) {
    for(i in 1:parms[["int_round"]]) {
      ...
      if(sigma_mat[1,i] == 0 | sigma_mat[2,i] == 0 | sigma_mat[3,i] == 0 | sigma_mat[4,i] == 0) {
...
testrun_flat <- ode_wrapper(y = init, func = amr, times = seq(0, 10000), parms = parms1)[[1]]
ode_wrapper(y = init, func = amr, times = seq(0, 10000), parms = parms1)[[2]]
...
for(i in 1:4) {
  ...
  single_tax(i, 0.5, parms1, init, amr, agg_func, ode_wrapper)[[2]]
}
