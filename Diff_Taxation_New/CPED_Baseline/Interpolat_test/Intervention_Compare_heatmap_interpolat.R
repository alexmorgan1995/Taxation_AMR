...
single_tax <- function(res_order, tax, parms, init, func, agg_func, ode_wrapper, approx_sigma) {
  ...
  #Real Model Run 
  run_real <- ode_wrapper(y = init, func = func, times = seq(0, 15000), parms = parms, approx_sigma)
  return(run_real)
}
...
testrun_flat <- list(ode_wrapper(y = init, func = amr, times = seq(0, 15000), parms = parms1, approx_sigma))
...
base <- ode_wrapper(y = init, func = amr, times = seq(0, 15000), parms = parms, approx_sigma)[[1]]
