...
parms = c(beta = 5, sigma_1 = 0.25, sigma_2 = 0.25, sigma_3 = 0.25,
          ...
          t_n = 3000, time_between = Inf, prop = 0.5)

# Run the Scenarios -------------------------------------------------------

#Flat Tax Model - ALL
#parms1 <- parms; parms1[grep("eff_tax1", names(parms1), value = TRUE)] <- 0.5
parms1[grep("eff_tax2", names(parms1), value = TRUE)] <- 0.5
parms1[grep("eff_tax3", names(parms1), value = TRUE)] <- 0.5
...
ggplot(data = test, aes(x = time, y = value, color = variable, lty = variable)) + geom_line(size = 1.2) + theme_bw() + 
  ...
  scale_color_manual(values = c("red", "blue", "green", "yellow","black", "black")) + 
