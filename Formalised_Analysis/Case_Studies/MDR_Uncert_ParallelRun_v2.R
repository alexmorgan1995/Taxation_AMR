...
colnames(parm_data_comb)[24:41] <- names(parms)[22:39]
...
mono_func <- function(n, parms_frame, init, amr_ode, usage_fun, multi_int_fun, low_parm, high_parm, agg_func, thresh) {
  ...
  if(values[4] == 0 & values[5] == 0 & values[6] == 0) {
    while(values[4] == 0 & values[5] == 0 & values[6] == 0) {
      parms_base[c(1:23)] <- runif(23, low_parm, high_parm)
      
      if(sum(parm_data[c("sigma1", "sigma2", "sigma3")]) > 1) {
        parm_data[c("sigma1", "sigma2", "sigma3")] <- parm_data[c("sigma1", "sigma2", "sigma3")]/
          (sum(parm_data[c("sigma1", "sigma2", "sigma3")]) + runif(1, 0, 1))
      }
...
comb_data_new <- data.frame(matrix(NA, nrow = nrow(comb_data), ncol = ncol(comb_data)))

for(i in 1:nrow(comb_data)) {
  comb_data_new[i,] <- unlist(comb_data[i,])
}
colnames(comb_data_new) <- colnames(comb_data)

#Update the Parameter Set 
parm_data_comb_new <- parm_data_comb
parm_data_comb_new[1:nrow(comb_data),1:20] <- comb_data_new[,31:50]

#Save the output

saveRDS(parm_data_comb_new, "/cluster/home/amorgan/Sens_Anal_Output/MDR_run_parms_v2.RDS")
saveRDS(comb_data_new, "/cluster/home/amorgan/Sens_Anal_Output/MDR_run_v2.RDS")
