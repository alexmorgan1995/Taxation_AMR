...
for(i in 1:3) {
  parms1 <- parms
  parms1[["eff_tax"]][i,] <- parms1[["base_tax"]]
  single_list[[i]] <- remNA_func(data.frame(ode(y = init, func = amr, times = seq(0, 10000), parms = parms1)))
}
...
parms_single1 <- parms; parms_single1[["eff_tax"]][1,]   <- 0.5
parms_single2 <- parms; parms_single2[["eff_tax"]][2,]  <- 0.5
parms_single3 <- parms; parms_single3[["eff_tax"]][3,]  <- 0.5
