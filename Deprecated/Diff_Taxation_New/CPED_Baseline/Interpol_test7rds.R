...
parms1 = list(lambda = 1/365*(2), int_round = 1, 
...
#We now need to identify a parameter combination which shows a highly effective 6th round of differential taxation.
#Remember this is specifically for the average resistance optimisation criteria. 
#Looks like Parameter Combination 58 is a good candidate. 

parms_dataframe <- readRDS("MDR_run_parms_interpol.RDS")
parms <- parms_dataframe[[58]]
parms[["eff_tax"]] <- parms1[["eff_tax"]]
...
for(i in 1:length(list_scen)) {
  ...
  colnames(data_agg)[4:7] <- c("High Res (HR)", "Medium Res (MR)", "Low Res (LR)", "Average Resistance") 
  melt_data[[i]] <- data.frame(melt(data_agg, id.vars = "time", measure.vars = colnames(data_agg)[4:7]),
                               "scen" = c("flat", "single1", "single2", "single3",
                                          "diff1", "diff2", "diff3", "diff4", "diff5", "diff6", "diff7")[i])
}
...
ggarrange(p_data[[1]], "", "",
          ...
          labels = c("A", "", "",
                     "B", "", "",
                     "C", "", "",
                     "", "", ""), hjust = -.1,  nrow = 5, ncol = 3, common.legend = T, legend = "bottom")
