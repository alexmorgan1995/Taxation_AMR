...
integral <- function(data, t_n, thresh){
  ...
  thresholds <- unlist(data[t_n-1, 7:8]* thresh)
  under_thresh <- sweep(data[data[,1] > t_n,][,7:8], 2, thresholds)
...
reduc_usage_vec <- sapply(1:length(list_usage), function(x) sum(list_usage[[x]][,5]))
...
OG_melt_rel$Var2 <- factor(rep(c("Flat Tax", "Single Tax (HR)", "Single Tax (LR)",
...
rescale_melt_rel$Var2 <- factor(rep(c("Flat Tax", "Single Tax (HR)",  "Single Tax (LR)",
                                      "Diff Tax (1 Rd)", "Diff Tax (2 Rd)", "Diff Tax (3 Rd)", "Diff Tax (4 Rd)",
                                      "Diff Tax (5 Rd)","Diff Tax (6 Rd)"), each = 3))
