...
integral <- function(data, t_n, thresh){
  ...
  prop_vec <- sum(under_50)/(10000 - t_n)
...
single_tax <- function(res_order, tax, parms, init, func, agg_func) {
  ...
  res_order_vec <- c(names(values_1rd)[which.max(values_1rd)],
                     names(values_1rd)[setdiff(1:3, c(which.min(values_1rd), which.max(values_1rd)))])[res_order]
...
#Flat Tax
parms1 <- parms; parms1[["eff_tax"]][,] <- parms1[["base_tax"]]
...
parms_singleMR <- parms; parms_singleMR[["eff_tax"]][as.numeric(substr(res_order_vec[2], 2, 2)), c(1:6)] <- 0.5
parms_singleLR <- parms; parms_singleLR[["eff_tax"]][as.numeric(substr(res_order_vec[3], 2, 2)), c(1:6)] <- 0.5
