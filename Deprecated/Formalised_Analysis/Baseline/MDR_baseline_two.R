...
integral <- function(data, t_n, thresh){
  ...
  data$aggR1 <- data$R1 + data$R12 
  data$aggR2 <- data$R2 + data$R12
  ...
  thresholds <- unlist(data[t_n-1, 6:7]* thresh)
  under_thresh <- sweep(data[data[,1] > t_n,][,6:7], 2, thresholds)
...
test_run_agg$AverageRes <- rowMeans(testrun_flat[,4:5])
test_run_agg$TotInf <- rowSums(testrun_flat[,3:5])

test_plot_flat <- melt(testrun_flat, id.vars = "time", measure.vars = colnames(testrun_flat)[4:5])
