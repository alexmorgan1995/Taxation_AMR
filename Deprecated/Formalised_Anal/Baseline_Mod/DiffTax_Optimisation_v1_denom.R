...
diff_tax_list_parms <- list()

for(i in 1:6) {
  diff_tax_list_parms[[i]] <- data.frame(multi_int_fun(i, 365*3, parms, init, amr)[[2]], 
                                   "scen" = paste0("diff", i))
}
...
parms_flat <- parms; parms_flat[grep("eff_tax", names(parms), value =T)]  <- parms[["base_tax"]] 
parms_single1 <- parms; parms_single1[grep("eff_tax1", names(parms), value =T)]  <- parms[["base_tax"]] 
parms_single2 <- parms; parms_single2[grep("eff_tax2", names(parms), value =T)]  <- parms[["base_tax"]] 
parms_single3 <- parms; parms_single3[grep("eff_tax3", names(parms), value =T)]  <- parms[["base_tax"]] 
parms_dual12 <- parms; parms_dual12[grep("eff_tax1|eff_tax2", names(parms), value =T)]  <- parms[["base_tax"]] 
parms_dual23 <- parms; parms_dual23[grep("eff_tax2|eff_tax3", names(parms), value =T)]  <- parms[["base_tax"]] 
parms_dual13 <- parms; parms_dual13[grep("eff_tax1|eff_tax2", names(parms), value =T)]  <- parms[["base_tax"]] 

parm_list <- list(parms_flat,parms_single1,parms_single2,parms_single3,parms_dual12,parms_dual23,parms_dual13)

over_parms <- append(parm_list, diff_tax_list_parms)

list_usage <- list()
for(i in 1:13) {
  list_usage[[i]] <- usage_fun(over_parms[[i]], list(testrun_flat, dual_list[[1]], dual_list[[2]], dual_list[[3]], 
                                                      single_list[[1]], single_list[[2]], single_list[[3]],
                                                      diff_tax_list[[1]], diff_tax_list[[2]], diff_tax_list[[3]],
                                                      diff_tax_list[[4]], diff_tax_list[[5]], diff_tax_list[[6]])[[i]])
}
