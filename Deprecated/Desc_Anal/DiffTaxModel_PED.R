...
multi_int_fun <- function(int_gen, basetax, time_between, parms, init, func){
  ...
  parms["time_between"] <- time_between
    ...
    parms[grep("eff_tax1", names(parms), value = T)] <- 1-(basetax*(values[1]/values[2])*parms[["PED1"]])
    parms[grep("eff_tax2", names(parms), value = T)] <- 1-(basetax*(values[2]/values[2])*parms[["PED2"]])
    parms[grep("eff_tax3", names(parms), value = T)] <- 1-(basetax*(values[3]/values[2])*parms[["PED3"]])
    parms[parms < 0] <- 0

    if(int_gen == 0){
      parms[grep("eff_tax1", names(parms), value = T)] <- 1-(basetax*parms[["PED1"]])
      parms[grep("eff_tax2", names(parms), value = T)] <- 1-(basetax*parms[["PED2"]])
      parms[grep("eff_tax3", names(parms), value = T)] <- 1-(basetax*parms[["PED3"]])
      parms[parms < 0] <- 0
    }
    print(parms)
    ...
    if(int_gen >= 2){
      ...
      parms[grep(paste0("eff_tax",substr(low_char, 2, 2)), names(parms), value = T)[-1]] <- 1-(basetax*(tail(testrun1[low_char],1)/values[2])*parms[["PED1"]])
      parms[grep(paste0("eff_tax",substr(med_char, 2, 2)), names(parms), value = T)[-1]] <- 1-(basetax*(tail(testrun1[med_char],1)/values[2])*parms[["PED2"]])
      parms[grep(paste0("eff_tax",substr(high_char, 2, 2)), names(parms), value = T)[-1]] <- 1-(basetax*(tail(testrun1[high_char],1)/values[2])*parms[["PED3"]])
      parms[parms < 0] <- 0
    ...
    if(int_gen >= 3){
      ...
      parms[grep(paste0("eff_tax",substr(low_char1, 2, 2)), names(parms), value = T)[-c(1,2)]] <- 1-(basetax*(tail(testrun2[low_char1],1)/values[2])*parms[["PED1"]])
      parms[grep(paste0("eff_tax",substr(med_char1, 2, 2)), names(parms), value = T)[-c(1,2)]] <- 1-(basetax*(tail(testrun2[med_char1],1)/values[2])*parms[["PED2"]])
      parms[grep(paste0("eff_tax",substr(high_char1, 2, 2)), names(parms), value = T)[-c(1,2)]] <- 1-(basetax*(tail(testrun2[high_char1],1)/values[2])*parms[["PED3"]])
      parms[parms < 0] <- 0
    ...
    if(int_gen >= 4){
      ...
      parms[grep(paste0("eff_tax",substr(low_char2, 2, 2)), names(parms), value = T)[-c(1,2,3)]] <- 1-(basetax*(tail(testrun3[low_char2],1)/values[2])*parms[["PED1"]])
      parms[grep(paste0("eff_tax",substr(med_char2, 2, 2)), names(parms), value = T)[-c(1,2,3)]] <- 1-(basetax*(tail(testrun3[med_char2],1)/values[2])*parms[["PED2"]])
      parms[grep(paste0("eff_tax",substr(high_char2, 2, 2)), names(parms), value = T)[-c(1,2,3)]] <- 1-(basetax*(tail(testrun3[high_char2],1)/values[2])*parms[["PED3"]])
      parms[parms < 0] <- 0
    ...
    if(int_gen >= 5){
      ...
      parms[grep(paste0("eff_tax",substr(low_char3, 2, 2)), names(parms), value = T)[-c(1,2,3,4)]] <- 1-(basetax*(tail(testrun4[low_char3],1)/values[2])*parms[["PED1"]])
      parms[grep(paste0("eff_tax",substr(med_char3, 2, 2)), names(parms), value = T)[-c(1,2,3,4)]] <- 1-(basetax*(tail(testrun4[med_char3],1)/values[2])*parms[["PED2"]])
      parms[grep(paste0("eff_tax",substr(high_char3, 2, 2)), names(parms), value = T)[-c(1,2,3,4)]] <- 1-(basetax*(tail(testrun4[high_char3],1)/values[2])*parms[["PED3"]])
      parms[parms < 0] <- 0
    ...
    if(int_gen >= 6){
      ...
      parms[grep(paste0("eff_tax",substr(low_char4, 2, 2)), names(parms), value = T)[-c(1,2,3,4,5)]] <- 1-(basetax*(tail(testrun5[low_char4],1)/values[2])*parms[["PED1"]])
      parms[grep(paste0("eff_tax",substr(med_char4, 2, 2)), names(parms), value = T)[-c(1,2,3,4,5)]] <- 1-(basetax*(tail(testrun5[med_char4],1)/values[2])*parms[["PED2"]])
      parms[grep(paste0("eff_tax",substr(high_char4, 2, 2)), names(parms), value = T)[-c(1,2,3,4,5)]] <- 1-(basetax*(tail(testrun5[high_char4],1)/values[2])*parms[["PED3"]])
      parms[parms < 0] <- 0
...
testrun_flat$AvgRes <- rowMeans(testrun_flat[4:6])
testrun_flat$TotInf <- rowSums(testrun_flat[3:6])
comb_prop_flat = melt(testrun_flat, id.vars = "time", measure.vars = c("W","R1","R2","R3"))

#Plotting Model Output

dyn_base <- ggplot(data = comb_prop_flat, aes(x = time, y = value*100, color = variable)) + geom_line() + theme_bw() + 
  scale_x_continuous(name = "Time (days)", expand = c(0, 0)) +  scale_y_continuous(name = "% Infected", expand = c(0, 0), limits = c(0,100)) + 
...
parms = c(beta = 5, sigma_1 = 0.25, sigma_2 = 0.25, sigma_3 = 0.25,
          ...
          mu_t = 1/7, eta_wr = 0.01, eta_rw= 0.01, 
          c1= 0.95, c2 = 0.94, c3 = 0.9,
          eff_tax1_1 = 0.5, eff_tax2_1 = 0.5, eff_tax3_1 = 0.5, 
          eff_tax1_2 = 0.5, eff_tax2_2 = 0.5, eff_tax3_2 = 0.5, 
          eff_tax1_3 = 0.5, eff_tax2_3 = 0.5, eff_tax3_3 = 0.5, 
          eff_tax1_4 = 0.5, eff_tax2_4 = 0.5, eff_tax3_4 = 0.5, 
          eff_tax1_5 = 0.5, eff_tax2_5 = 0.5, eff_tax3_5 = 0.5, 
          eff_tax1_6 = 0.5, eff_tax2_6 = 0.5, eff_tax3_6 = 0.5, 
          PED1 = 1.5, PED2 = 1, PED3 = 0.5,
          t_n = 3000, time_between = Inf)

test_flat <- multi_int_fun(0, 0.5, 365*3, parms, init, amr)
...
dyn_flat <- ggplot(data = comb_prop_flat, aes(x = time, y = value*100, color = variable)) + geom_line() + theme_bw() + 
  annotate("rect", xmin=c(parms[["t_n"]]), xmax= c(Inf), ymin=c(0), ymax=c(Inf), alpha= 0.2, fill=rep("red")) + 
  scale_x_continuous(name = "Time (days)", expand = c(0, 0)) +  scale_y_continuous(name = "% Infected", expand = c(0, 0), limits = c(0,100)) + 
  theme(legend.text=element_text(size=12), legend.title = element_blank(), axis.text=element_text(size=12), 
        axis.title.y=element_text(size=12), axis.title.x = element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
        legend.spacing.x = unit(0.3, 'cm'), legend.position = "bottom") + 
...
tax_data_flat <- data.frame("time" = seq(0, 10000),
                       "R1" = c(rep(1, 3001), rep(0.5, 7000)), 
                       "R2" = c(rep(1, 3001), rep(0.5, 7000)), 
                       "R3" = c(rep(1, 3001), rep(0.5, 7000)))

comb_tax_flat = melt(tax_data_flat, id.vars = "time", measure.vars = c("R1","R2","R3"))

tax_flat <- ggplot(data = comb_tax_flat, aes(x = time, y = (1-value)*100, color = variable)) + geom_line(size = 1.2) + theme_bw() + 
  annotate("rect", xmin=c(parms[["t_n"]]), xmax= c(Inf), ymin=c(0), ymax=c(Inf), alpha= 0.2, fill=rep("red")) + 
  scale_x_continuous(name = "Time (days)", expand = c(0, 0)) +  scale_y_continuous(name = "Effective Tax Rate (%)", expand = c(0, 0), limits = c(0,100)) + 
  theme(legend.text=element_text(size=12), legend.title = element_blank(), axis.text=element_text(size=12), 
        axis.title.y=element_text(size=12), axis.title.x = element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
        legend.spacing.x = unit(0.3, 'cm'), legend.position = "bottom") + 
  scale_color_manual(values = c("red", "blue", "green"))

#Plotting Effective Usage 

tax_data_flat <- data.frame("time" = seq(0, 10000),
                            "R1" = c(rep(1, 3001), rep(1-(0.5*parms[["PED1"]]), 7000)), 
                            "R2" = c(rep(1, 3001), rep(1-(0.5*parms[["PED2"]]), 7000)), 
                            "R3" = c(rep(1, 3001), rep(1-(0.5*parms[["PED3"]]), 7000)))

comb_tax_flat = melt(tax_data_flat, id.vars = "time", measure.vars = c("R1","R2","R3"))

tax_usage <- ggplot(data = comb_tax_flat, aes(x = time, y = (1-value)*100, color = variable)) + geom_line(size = 1.2) + theme_bw() + 
  annotate("rect", xmin=c(parms[["t_n"]]), xmax= c(Inf), ymin=c(0), ymax=c(Inf), alpha= 0.2, fill=rep("red")) + 
  scale_x_continuous(name = "Time (days)", expand = c(0, 0)) +  scale_y_continuous(name = "Effective Reduction in Usage (%)", expand = c(0, 0), limits = c(0,100)) + 
  theme(legend.text=element_text(size=12), legend.title = element_blank(), axis.text=element_text(size=12), 
        axis.title.y=element_text(size=12), axis.title.x = element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
        legend.spacing.x = unit(0.3, 'cm'), legend.position = "bottom") + 
  scale_color_manual(values = c("red", "blue", "green"))

ggarrange(dyn_flat, tax_flat, tax_usage, nrow = 3, ncol = 1, common.legend = T, legend = "bottom")

# Run a Single Diff Model -------------------------------------------------

test_single <- multi_int_fun(1, 0.5, 365*3, parms, init, amr)
test_singledata <- test_single[[1]]
comb_prop = melt(test_singledata, id.vars = "time", measure.vars = c("W","R1","R2","R3"))

dyn_single <- ggplot(data = comb_prop, aes(x = time, y = value*100, color = variable)) + geom_line() + theme_bw() + 
  annotate("rect", xmin=c(parms[["t_n"]]), xmax= c(Inf), ymin=c(0), ymax=c(Inf), alpha= 0.2, fill=rep("red")) + 
  scale_x_continuous(name = "Time (days)", expand = c(0, 0)) +  scale_y_continuous(name = "% Infected", expand = c(0, 0), limits = c(0,100)) + 
  theme(legend.text=element_text(size=12), legend.title = element_blank(), axis.text=element_text(size=12), 
        axis.title.y=element_text(size=12), axis.title.x = element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
...
taxrate_single <- ggplot(tax_melt, aes(x = time, y = (1-value)*100, color = variable)) + geom_line(size = 1.2) + theme_bw() + 
  annotate("rect", xmin=c(parms[["t_n"]]), xmax= c(Inf), ymin=c(0), ymax=c(Inf), alpha= 0.2, fill=rep("red")) + 
  scale_x_continuous(name = "Time (days)", expand = c(0, 0)) +  scale_y_continuous(name = "Effective Reduction in Usage (%)", expand = c(0, 0), limits = c(0,101)) + 
  theme(legend.text=element_text(size=12), legend.title = element_blank(), axis.text=element_text(size=12), 
        axis.title.y=element_text(size=12), axis.title.x = element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
        legend.spacing.x = unit(0.3, 'cm'), legend.position = "bottom") + 
  scale_color_manual(values = c("red", "blue", "green"))

ggarrange(dyn_single, taxrate_single, nrow = 2, ncol = 1, common.legend = T, legend = "bottom")
...
totinf_data <- data.frame("time" = seq(0, 10000),
                          "FlatTax" = rowSums(testrun_flat[3:6]), 
                          "SingleDiffTax" = rowSums(test_singledata[3:6]), 
                          "DiffTax" = rowSums(test[3:6]))
                          
totinfmelt <- melt(totinf_data, id.vars = c("time"), measure.vars = c("FlatTax", "SingleDiffTax", "DiffTax"))  

totinf <- ggplot(totinfmelt, aes(x = time, y = value*100, color = variable)) + geom_line(size = 1.2) + theme_bw() + 
  annotate("rect", xmin=c(parms[["t_n"]], parms[["t_n"]] + (365*3), parms[["t_n"]] + (365*3)*2, parms[["t_n"]] + (365*3)*3, parms[["t_n"]] + (365*3)*4, parms[["t_n"]] + (365*3)*5), 
           xmax= c(parms[["t_n"]] + (365*3), parms[["t_n"]] + (365*3)*2, parms[["t_n"]] + (365*3)*3, parms[["t_n"]] + (365*3)*4, parms[["t_n"]] + (365*3)*5, Inf), 
           ymin=c(rep(95, 6)), ymax=c(rep(98, 6)), alpha=rep(c(0.2, 0.125), 3), fill=rep("red", 6)) + 
  scale_x_continuous(name = "Time (days)", expand = c(0, 0)) + 
  theme(legend.text=element_text(size=12), legend.title = element_blank(), axis.text=element_text(size=12), 
        axis.title.y=element_text(size=12), axis.title.x = element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
        legend.spacing.x = unit(0.3, 'cm'), legend.position = "bottom") + 
  scale_y_continuous(name = "Total Infections (%)", expand = c(0, 0), limits = c(95, 98)) 

# Compare the Average Resistance ------------------------------------------

totres_data <- data.frame("time" = seq(0, 10000),
                          "FlatTax" = rowMeans(testrun_flat[4:6]), 
                          "SingleDiffTax" = rowMeans(test_singledata[4:6]), 
                          "DiffTax" = rowMeans(test[4:6]))

totres_melt <- melt(totres_data, id.vars = c("time"), measure.vars = c("FlatTax", "SingleDiffTax", "DiffTax"))   

avgres <- ggplot(totres_melt, aes(x = time, y = value*100, color = variable)) + geom_line(size = 1.2) + theme_bw() +
  scale_y_continuous(name = "Average Resistance (%)", expand = c(0, 0), limits = c(0, 40)) + 
  annotate("rect", xmin=c(parms[["t_n"]], parms[["t_n"]] + (365*3), parms[["t_n"]] + (365*3)*2, parms[["t_n"]] + (365*3)*3, parms[["t_n"]] + (365*3)*4, parms[["t_n"]] + (365*3)*5), 
           xmax= c(parms[["t_n"]] + (365*3), parms[["t_n"]] + (365*3)*2, parms[["t_n"]] + (365*3)*3, parms[["t_n"]] + (365*3)*4, parms[["t_n"]] + (365*3)*5, Inf), 
           ymin=c(rep(0, 6)), ymax=c(rep(40, 6)), alpha=rep(c(0.2, 0.125), 3), fill=rep("red", 6)) + 
  scale_x_continuous(name = "Time (days)", expand = c(0, 0)) + 
  theme(legend.text=element_text(size=12), legend.title = element_blank(), axis.text=element_text(size=12), 
        axis.title.y=element_text(size=12), axis.title.x = element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
        legend.spacing.x = unit(0.3, 'cm'), legend.position = "bottom") 
...
inf_int = data.frame("flat" = integral(testrun_flat, 3000), 
           "single" = integral(test_singledata, 3000),
           "diff" = integral(test, 3000))

write.csv(inf_int, file = "/Users/amorgan/Documents/PostDoc/PrelimAnalysis/Theoretical_Analysis/Model_Output/integrals_PED.csv")

rownames(inf_int) <- c("totinf", "resinf", "wt", "avgres", "R1", "R2" , "R3")


# Combine Integral Plots --------------------------------------------------

ggarrange(totinf, avgres, nrow = 2, ncol = 1, common.legend = T, legend = "bottom")
