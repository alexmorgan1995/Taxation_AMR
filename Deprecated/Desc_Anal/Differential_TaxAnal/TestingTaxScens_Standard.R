...
flat_absolute <- data.frame("time" = base$time,
                            "flat_totinf" = rowSums(testrun_flat[3:6]),
                            "flat_avgres" = rowMeans(testrun_flat[4:6]),
                            "usage" = c(rep(0.25*3, 3000),rep((0.25*3)*0.5, 7001)))

totinf_abs <- ggplot(flat_absolute, aes(time, flat_totinf)) + geom_line(size = 1.2) + theme_bw() + 
  scale_y_continuous(name = "Total Infections", limits = c(0,1)) + 
  geom_ribbon(aes(ymax=c(rep(0,3000), rowSums(testrun_flat[3:6])[3000: 10000]), 
                  ymin=0), fill = "red", alpha = 0.2)

res_abs <- ggplot(flat_absolute, aes(time, flat_avgres)) + geom_line(size = 1.2) + theme_bw() + 
  scale_y_continuous(name = "Average Resistance") + 
  geom_ribbon(aes(ymax=c(rep(0,3000), rowMeans(testrun_flat[4:6])[3000: 10000]), ymin = 0), fill = "red", alpha = 0.2)

usage_abs <- ggplot(flat_absolute, aes(time, usage)) + geom_line(size = 1.2) + theme_bw() + 
  scale_y_continuous(name = "Fraction of Popiulation Treated") + 
  geom_ribbon(aes(ymax=c(rep(0,3000), rep((0.25*3)*0.5, 7001)), ymin = 0), fill = "red", alpha = 0.2)
