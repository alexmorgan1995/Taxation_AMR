...
ggplot(data = test, aes(x = time, y = value, color = variable, lty = variable)) + geom_line(size = 1.2) + theme_bw() + 
  ...
  scale_color_manual(values = c("red", "blue", "green", "Orange","black", "black")) + 
  scale_linetype_manual(values = c(1, 1, 1, 1, 6, 3))
...
for(i in 1:3) {
  parms1 <- parms
  parms1[grep(paste0("eff_tax", dual_p[i,][1], "|", "eff_tax", dual_p[i,][2]), names(parms1), value = TRUE)] <- 0.25
...
for(i in 1:3) {
  test <- melt(dual_list[[i]], id.vars = c("time"), measure.vars = c(colnames(testrun_flat)[3:8]))
  p_list_dual[[i]] <- ggplot(test, aes(time, value, color = variable, lty = variable)) + geom_line(size = 1.2) + theme_bw() + 
    annotate("rect", xmin=c(parms[["t_n"]]), xmax= c(Inf), ymin=c(0), ymax=c(Inf), alpha= 0.2, fill=rep("red")) + 
    scale_x_continuous(name = "Time (days)", expand = c(0, 0)) +  scale_y_continuous(name = "Proportion Infected", expand = c(0, 0), limits = c(0,1)) + 
    theme(legend.text=element_text(size=12), legend.title = element_blank(), axis.text=element_text(size=12), 
          axis.title.y=element_text(size=12), axis.title.x = element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
          legend.spacing.x = unit(0.3, 'cm'), legend.position = "bottom") + 
    scale_color_manual(values = c("red", "blue", "green", "orange","black", "black")) + 
    scale_linetype_manual(values = c(1, 1, 1, 1, 6, 3)) + labs(title = paste0("Dual Tax = Class ", dual_p[i,][1]," | Class ", dual_p[i,][2] ))
}

ggarrange(plotlist = p_list_dual, ncol = 1 ,nrow = 3, common.legend = T, legend = "bottom")
...
for(i in 1:3) {
  ...
  dat <- ode(y = init, func = amr, times = seq(0, 10000), parms = parms1)
  single_list[[i]] <- data.frame(dat,
                                 "AverageRes" = rowMeans(dat[,4:6]),
                                 "TotInf" = rowSums(dat[,3:6]),
...
p_list_single <- list()
for(i in 1:3) {
  test <- melt(single_list[[i]], id.vars = c("time"), measure.vars = c(colnames(testrun_flat)[3:8]))
  
  p_list_single[[i]] <- ggplot(test, aes(time, value, color = variable, lty = variable)) + geom_line(size = 1.2) + theme_bw() + 
    annotate("rect", xmin=c(parms[["t_n"]]), xmax= c(Inf), ymin=c(0), ymax=c(Inf), alpha= 0.2, fill=rep("red")) + 
    scale_x_continuous(name = "Time (days)", expand = c(0, 0)) +  scale_y_continuous(name = "Proportion Infected", expand = c(0, 0), limits = c(0,1)) + 
    theme(legend.text=element_text(size=12), legend.title = element_blank(), axis.text=element_text(size=12), 
          axis.title.y=element_text(size=12), axis.title.x = element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
          legend.spacing.x = unit(0.3, 'cm'), legend.position = "bottom") + 
    scale_color_manual(values = c("red", "blue", "green", "orange","black", "black")) + 
    scale_linetype_manual(values = c(1, 1, 1, 1, 6, 3)) + labs(title = paste0("Single Tax = Class ", i))
}

ggarrange(plotlist = p_list_single, ncol = 1 ,nrow = 3, common.legend = T, legend = "bottom")
...
p_list_diff <- list()
for(i in 1:6) {
  test <- melt(diff_tax_list[[i]], id.vars = c("time"), measure.vars = c(colnames(testrun_flat)[3:8]))
  
  p_list_diff[[i]] <- ggplot(test, aes(time, value, color = variable, lty = variable)) + geom_line(size = 1.2) + theme_bw() + 
    annotate("rect", xmin=c(parms[["t_n"]]), xmax= c(Inf), ymin=c(0), ymax=c(Inf), alpha= 0.2, fill=rep("red")) + 
    scale_x_continuous(name = "Time (days)", expand = c(0, 0)) +  scale_y_continuous(name = "Proportion Infected", expand = c(0, 0), limits = c(0,1)) + 
    theme(legend.text=element_text(size=12), legend.title = element_blank(), axis.text=element_text(size=12), 
          axis.title.y=element_text(size=12), axis.title.x = element_text(size=12), plot.margin = unit(c(0.35,1,0.35,1), "cm"),
          legend.spacing.x = unit(0.3, 'cm'), legend.position = "bottom") + 
    scale_color_manual(values = c("red", "blue", "green", "orange", "black", "black")) + 
    scale_linetype_manual(values = c(1, 1, 1, 1, 6, 3)) + labs(title = paste0("Differential Tax (no of rounds =  ", i, ")"))
}

ggarrange(plotlist = p_list_diff, ncol = 2 ,nrow = 3, common.legend = T, legend = "bottom")
