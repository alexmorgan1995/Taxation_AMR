...
mono_func <- function(n, parms_frame, init, amr_ode, usage_fun, multi_int_fun, low_parm, high_parm, agg_func, thresh, ode_wrapper, approx_sigma, ban_wrapper) {
  ...
  #Run Baseline
  run_base <- ode_wrapper(y = init, func = amr, times = seq(0, 10300), parms = parms_base, approx_sigma)[[1]]
  ...
  for(i in 1:2){
    ...
    if(i == 1 ) {
      parms[["eff_tax"]][as.numeric(substr(res_order_vec[i], 2, 2)), c(1:6)] <- parms[["base_tax"]]
      parms[["int_round"]] <- 1
    ...
    if(i == 2) {
      ban <- ban_wrapper(times = seq(0, 10300), init, parms, amr_ode, approx_sigma, ban = i-1)
      out <- ban[[1]]
      parms <- ban[[2]]
    }
  ...
  names(output) <- c("singleHR_inf", "banHR_inf",  
                     "singleHR_res", "banHR_res", 
                     "singleHR_shan", "banHR_shan", 
                     "singleHR_avganti", "banHR_avganti", 
...
for(i in 1:100) {
  ...
  parm_data_comb$base_tax <- i/100
  print(i)
  uncert_list[[i]] <- mclapply(1:1000, 
                   FUN = mono_func, 
...
saveRDS(uncert_list, "/cluster/home/amorgan/Sens_Anal_Output/uncert_thresh.RDS")
