...
#Shannon's
win_shan <- round((win_import[,31:45]), 3)
...
m_res$factors <- c(rep("Taxation", 11000), rep("Ban", 4000))
...
for(i in 1:15) {
  prop_1000 <- win_import_change[,c(16:30)[i]]
...
m_inf$factors <- c(rep("Taxation", 11000), rep("Ban", 4000))
...
m_avganti$factors <- c(rep("Taxation", 11000), rep("Ban", 4000))
...
box_avganti <- ggplot(m_avganti, aes(x=variable, y=value, fill = variable, alpha = variable)) + coord_cartesian(ylim=c(-0.0001, 4)) +
