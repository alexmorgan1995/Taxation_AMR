...
vec_fail <- do.call("rbind", lapply(list(winintfail_base, winintfail_tax25, winintfail_tax75, winintfail_lowComp, winintfail_highComp,
                                         winintfail_tax35, winintfail_tax10, winintfail_tax05, winintfail_twoclass, winintfail_fourclass), intfail))
...
res_frame$Int <- as.factor(rep(c(rep("Tax", 11),rep("Bans", 4)), 10))
...
res_plot <- ggplot(res_frame, aes(Interventions, scen)) + theme_bw() +
  geom_tile(aes(fill = Resistance_Scale_v1)) + scale_fill_distiller(palette ="Blues", direction = 1) +
  facet_grid(factor~ Int, scales  = "free", space = "free") +
...
avganti_frame$Int <- as.factor(rep(c(rep("Tax", 11),rep("Bans", 4)), 10))
...
avg_anti_plot <- ggplot(avganti_frame, aes(Interventions, scen)) + theme_bw() +
  geom_tile(aes(fill = avganti_frame_v1)) + scale_fill_distiller(palette ="Blues", direction = 1) +
  facet_grid(factor~ Int, scales  = "free", space = "free") +
