...
box_avganti <- ggplot(m_avganti, aes(x=variable, y=value, fill = variable, alpha = variable)) + coord_cartesian(ylim=c(-0.0001, 3)) + 
  facet_grid(. ~ factors, scales = "free", space = "free") +
  geom_boxplot(outlier.shape = NA, show.legend = FALSE, fill = "red") + theme_bw() + labs(y = "Number of Available Antibiotics", x = "")  + 
  scale_alpha_manual(values=  prop_vec$prop_inc) +
  theme(legend.position= "bottom", legend.text=element_text(size=11), legend.title =element_text(size=12), axis.text=element_text(size=11), 
...
inc_avganti_onlytax  <- win_avganti[,1:10]
m_avganti <- melt(inc_avganti_onlytax, measure.vars = colnames(inc_avganti_onlytax))

inc_avganti_onlytax[is.na(inc_avganti_onlytax)] <- 0
inc_avganti_onlytax <- inc_avganti_onlytax[rowSums(inc_avganti_onlytax[, -1]) > 0, ]

win_avganti_trans <- t(apply(inc_avganti_onlytax, 1, function(x) {
  val = max(x)
  x[is.na(x)] <- 0
  x[x != val] <- 0
  x[x == val] <- 1
...
#Win Heat Map
win_avganti_p_onlytax <- ggplot(prop_win_avganti, aes(Interventions, "")) + theme_bw() +
  geom_tile(aes(fill = Average_Anti)) +
  geom_text(aes(label=Average_Anti, color = Interventions)) + 
  scale_colour_manual(values=prop_win_avganti$Color) +
  scale_fill_distiller(palette ="Blues", direction = 1) +
...
box_avganti_onlytax <- ggplot(m_avganti, aes(x=variable, y=value, fill = variable, alpha = variable)) + coord_cartesian(ylim=c(-0.0001, 3)) + 
  geom_boxplot(outlier.shape = NA, show.legend = FALSE, fill = "red") + theme_bw() + labs(y = "Number of Available Antibiotics", x = "")  + 
  scale_alpha_manual(values=  prop_vec$prop_inc) +
  theme(legend.position= "bottom", legend.text=element_text(size=11), legend.title =element_text(size=12), axis.text=element_text(size=11), 
        axis.title.y=element_text(size=12), axis.title.x= element_blank(), plot.margin = unit(c(0.3,1,0,2), "cm"),
