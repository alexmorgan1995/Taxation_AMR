...
win_import_change <- readRDS("MDR_run_Base_tax_noinc.RDS"); win_import <- win_import_change
win_import_change <- readRDS("MDR_run_Base_tax_inc.RDS"); win_import <- win_import_change
...
#Average Antibiotics
win_avganti <- round((win_import[,22:28]), 3)

# Resistance --------------------------------------------------------------

inc_res <- win_res
m_res <- melt(inc_res, measure.vars = colnames(inc_res))
m_res$factors <- c(rep("Taxation", 4000), rep("Ban", 3000))
...
for(i in 1:7) {
  prop_1000 <- win_import_change[,c(8:14)[i]]
...
prop_win_res$factors <- c(rep("Taxation", 4) , rep("Ban", 3))
...
m_inf$factors <- c(rep("Taxation", 4000), rep("Ban", 3000))
...
prop_win_inf$factors <- c(rep("Taxation", 4), rep("Ban", 3))
...
m_avganti$factors <- c(rep("Taxation", 4000), rep("Ban", 3000))
...
prop_win_avganti$factors <- c(rep("Taxation", 4), rep("Ban", 3))
...
ggsave(test, filename = "run_noinc.png", dpi = 300, width = 11, height = 14, units = "in",
