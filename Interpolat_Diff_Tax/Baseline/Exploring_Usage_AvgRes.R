...
testrun_flat <- list(ode_wrapper(y = init, func = amr, times = seq(0, 10300), parms = parms1, approx_sigma))
...
for(i in 1:3) {
  parms1 <- parms
  single_list[[i]] <- single_tax(i, 0.5, parms1, init, amr, agg_func, ode_wrapper, approx_sigma)
}

#Diff Tax
diff_tax_list <- list()
for(i in 1:6) {
  diff_tax_list[[i]] <- multi_int_fun(i, 365*3, parms, init, amr, agg_func, ode_wrapper, approx_sigma)
}

#Bans
ban_list <- list()
for(i in 1:3) {
  ban_list[[i]] <- ban_wrapper(times = seq(0, 10300), init, parms, amr, approx_sigma, ban = i)
}
...
for(i in 1:length(list_scen)) {
  data_agg <- agg_func(list_scen[[i]][[1]]) 
  colnames(data_agg)[4:6] <- c("Antibiotic with Highest Res (HR)", "Antibiotic with Medium Res (MR)", "Antibiotic with Lowest Res (LR)") 
  data_agg$average <- rowMeans(data_agg[4:6])
  melt_data[[i]] <- data.frame(melt(data_agg, id.vars = "time", measure.vars = colnames(data_agg)[7]),
...
for(i in 1:length(melt_data)) {
  data <- melt_data[[i]]
  p_data[[i]] <- ggplot(data, aes(time/365, value, color = variable)) + 
    annotate("rect",xmin = 3000/365, xmax = Inf, ymin = 0, ymax = Inf, fill = "red", alpha = 0.075) +
    geom_line() + theme_bw() + theme(legend.position = "bottom") +
    labs(x = "Time (Years)", y = "Prevalence", title = c("Flat Tax",
    ...
    scale_y_continuous(limits = c(0.2,0.3), expand = c(0,0)) + scale_x_continuous(expand = c(0,0), breaks = seq(0, 50, by = 5)) +
    theme(legend.text = element_text(size=10), panel.grid = element_blank(),
...
for(i in 1:length(list_scen)) {
  ...
  sigma_data$average <- rowMeans(sigma_data[2:4])
  melt_data_sigma[[i]] <- data.frame(melt(sigma_data, id.vars = "time", measure.vars = colnames(sigma_data)[5]),
                               "scen" = c("flat", "single1", "single2", "single3",
                                          "diff1", "diff2", "diff3", "diff4", "diff5", "diff6",
                                          "ban1", "ban2", "ban3")[i])
}
...
#Plotting Loop
for(i in 1:length(melt_data_sigma)) {
  data <- melt_data_sigma[[i]]
  p_sigma_data[[i]] <- ggplot(data, aes(time/365, value, color = variable)) + 
    annotate("rect",xmin = 3000/365, xmax = Inf, ymin = 0, ymax = Inf, fill = "red", alpha = 0.075) +
    geom_line() + theme_bw() + theme(legend.position = "bottom") +
    labs(x = "Time (Years)", y = "% Overall Population Usage", title = c("Flat Tax",
                                                         "Single Tax (HR)","Single Tax (MR)","Single Tax (LR)",
                                                         "Diff Tax (1 Rd)", "Diff Tax (2 Rd)", "Diff Tax (3 Rd)",
                                                         "Diff Tax (4 Rd)", "Diff Tax (5 Rd)", "Diff Tax (6 Rd)",
                                                         "Ban (HR)", "Ban (MR)", "Ban (LR)")[i], color = "") +
...
ggarrange(p_sigma_data[[1]], "", "",
          p_sigma_data[[2]], p_sigma_data[[3]],p_sigma_data[[4]], 
          p_sigma_data[[5]], p_sigma_data[[6]], p_sigma_data[[7]],
          p_sigma_data[[8]], p_sigma_data[[9]], p_sigma_data[[10]],
          p_sigma_data[[11]], p_sigma_data[[12]], p_sigma_data[[13]],
          labels = c("A", "", "",
                     "B", "", "",
                     "C", "", "",
                     "", "", "",
