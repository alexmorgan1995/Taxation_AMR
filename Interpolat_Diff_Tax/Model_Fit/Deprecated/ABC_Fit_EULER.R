library("deSolve"); library("bayestestR"); library("tmvtnorm")
rm(list=ls())
...
ABC_algorithm <- function(N, G, fitmodel, base_parms, init.state, ode_wrapper, approx_sigma)  {
  N_ITER_list <- list()
  ...
  for(g in 1:G) {
    ...
    while(i <= N) {
      ...
      if(prior.non.zero(c(d_beta, d_eta_wr, d_eta_rw, d_eta_rr_rrr, d_c1, d_c2, d_c3, d_c12, d_c13, d_c23, d_c123))) {
        ...
        thetaparm <- list(lambda = 1/365*(2), int_round = 1, 
                         ...
                         eta_rr = d_eta_rr_rrr, eta_rrr = d_eta_rr_rrr,  
        ...
        if((dist[1] <= epsilon_R1[g]) && (dist[2] <= epsilon_R2[g]) && (dist[3] <= epsilon_R3[g]) && (!is.na(dist))) {
          # Store results
          
          res.new[i,]<-c(d_beta, d_eta_wr, d_eta_rw, d_eta_rr_rrr, d_c1, d_c2, d_c3, d_c12, d_c13, d_c23, d_c123)
          dist_data[i,] <- dist
          # Calculate weights
          if(g==1){
    ...
    colnames(res.new) <- c("beta", "eta_wr", "eta_rw", "eta_rr_rrr", "c1", "c2", "c3", "c12", "c13", "c23", "c123")
    write.csv(res.new, file = paste("/cluster/home/amorgan/Model_Fit_Output/ABC_",g,".csv",sep=""), row.names=FALSE)
...
N <- 200 #(ACCEPTED PARTICLES PER GENERATION)

lm.low <- c(0, 0, 0, 0, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5)
lm.upp <- c(10, 0.5, 0.5, 0.5, 1, 1, 1, 1, 1, 1, 1) #Upper and lower bounds for the priors - for the multivariate normal dist pert kernel
...
#Thresholds 
epsilon_R1 <- c(0.5*1, 0.5*0.8, 0.5*0.6, 0.5*0.4, 0.5*0.3, 0.5*0.2, 0.5*0.15, 0.5*0.1, 0.5*0.075, 0.5*0.05)
epsilon_R2 <- c(0.25*1, 0.25*0.8, 0.25*0.6, 0.25*0.4, 0.25*0.3, 0.25*0.2, 0.25*0.15, 0.25*0.1, 0.25*0.075, 0.25*0.05)
epsilon_R3 <- c(0.1*1, 0.1*0.8, 0.1*0.6, 0.1*0.4, 0.1*0.3, 0.1*0.2, 0.1*0.15, 0.1*0.1, 0.1*0.075, 0.1*0.05)

#Run the model 
start_time <- Sys.time()

dist_save <- ABC_algorithm(N = 200, 
                           G = 10,
                           fitmodel = amr, 
...
saveRDS(dist_save, file = "/cluster/home/amorgan/Model_Fit_Output/dist_list.rds")
