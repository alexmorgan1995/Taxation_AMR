...
ABC_algorithm <- function(N, G, single_run, distanceABC, fitmodel, init.state, epsilon, lm.low, lm.upp, thetaparm, ode_wrapper, approx_sigma)  {
  ...
  for(g in 1:G) {
    ...
    write.csv(res.old, file =  paste("/cluster/home/amorgan/Model_Fit_Output/ABC_v2_",g,".csv",sep=""), row.names=FALSE)
  }
  return(out)
}
...
ABC_algorithm(N = 1000,
              ...
              init.state = c(X = 0.99, Wt = 1-0.99, R1 = 0, R2 = 0, R3 = 0,
                             R12 = 0, R13 = 0, R23 = 0,
                             R123 = 0), 
              epsilon = list("R1" =  c(0.5*1, 0.5*0.8, 0.5*0.6, 0.5*0.4, 0.5*0.3, 0.5*0.2, 0.5*0.15, 0.5*0.1, 0.5*0.05, 0.5*0.01),
                             "R2" = c(0.25*1, 0.25*0.8, 0.25*0.6, 0.25*0.4, 0.25*0.3, 0.25*0.2, 0.25*0.15, 0.25*0.1, 0.25*0.05, 0.25*0.01),
                             "R3" =  c(0.1*1, 0.1*0.8, 0.1*0.6, 0.1*0.4, 0.1*0.3, 0.1*0.2, 0.1*0.15, 0.1*0.1, 0.1*0.05, 0.1*0.01)),
...
saveRDS(test, file = "/cluster/home/amorgan/Model_Fit_Output/dist_list_v2.rds")
