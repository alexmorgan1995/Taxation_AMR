...
parm_data <- data.frame(t(replicate(10000, runif(15, low_parm, high_parm))))
...
mono_func <- function(n, parms_frame, init, amr_ode, usage_fun, multi_int_fun, low_parm, high_parm, agg_func, thresh, ode_wrapper, approx_sigma) {
  ...
  if(values[4] == 0 & values[5] == 0) {
    while(values[4] == 0 & values[5] == 0) {
      parms_base[c(1:15)] <- as.list(runif(15, low_parm, high_parm))
      ...
      parms_base["c12"] <- as.list(sort(as.numeric(parms_base[c("c1", "c2", "c12")]), decreasing = T)[3])
  ...
  for(i in 1:11){
    ...
    if(i >= 10 & i <= 11) {
      ...
      ban <- ban_wrapper(times = seq(0, 10300), init, parms, amr_ode, approx_sigma, ban = i-9)
      out <- ban[[1]]
      parms <- ban[[2]]
    }
...
parm_data_comb_new[1:nrow(comb_data),1:19] <- comb_data[,45:62]
