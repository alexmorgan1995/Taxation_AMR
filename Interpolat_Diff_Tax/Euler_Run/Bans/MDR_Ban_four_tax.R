...
approx_sigma <- function(sigma_mat){
  ...
  usage = data.frame("time" = seq(0,10300),
                     ...
                     "PopUsage4" = c(rep(sigma_mat[4,1], 3000),
                                     rep(sigma_mat[4,2], 365*3), rep(sigma_mat[4,3], 365*3), rep(sigma_mat[4,4], 365*3),
                                     rep(sigma_mat[4,5], 365*3), rep(sigma_mat[4,6], 365*3), rep(sigma_mat[4,7], 10301 - (3000 + (365*3)*5))))
  return(usage)
}
...
ban_wrapper <- function(times, init, parms, func, approx_sigma, ban) {
  ...
  if(ban >= 1 & ban <= 4) {
    ...
    sigma_mat[,2:7] = c(sigma_mat[1,1]*(1 + ((ban_vec[1]*PED[1,1]) + (ban_vec[2]*PED[2,1]) + (ban_vec[3]*PED[3,1]) + (ban_vec[4]*PED[4,1]))),
                        sigma_mat[2,1]*(1 + ((ban_vec[1]*PED[1,2]) + (ban_vec[2]*PED[2,2]) + (ban_vec[3]*PED[3,2]) + (ban_vec[4]*PED[4,2]))),
                        sigma_mat[3,1]*(1 + ((ban_vec[1]*PED[1,3]) + (ban_vec[2]*PED[2,3]) + (ban_vec[3]*PED[3,3]) + (ban_vec[4]*PED[4,3]))),
                        sigma_mat[4,1]*(1 + ((ban_vec[1]*PED[1,4]) + (ban_vec[2]*PED[2,4]) + (ban_vec[3]*PED[3,4]) + (ban_vec[4]*PED[4,4]))))
    sigma_mat[as.numeric(substr(res_order_vec, 2, 2)),2:7] <- 0
     }
  
  if(ban == "all") {
    sigma_mat[,2:7] = c(0,0,0,0)
  }
...
usage_fun <- function(parms){
  ...
  if(parms[["int_round"]] == 0) {
    usage = data.frame("time" = seq(0,7300),
                       ...
                       "PopUsage4" = rep(parms[["sigma_mat"]][4,2], 7301))
  }
  ...
  if(parms[["int_round"]] > 0) {
    usage = data.frame("time" = seq(0,7300),
                       ...
                       "PopUsage4" = c(rep(parms[["sigma_mat"]][4,2] , 365*3), rep(parms[["sigma_mat"]][4,3] , 365*3), rep(parms[["sigma_mat"]][4,4] , 365*3),
                                       rep(parms[["sigma_mat"]][4,5], 365*3), rep(parms[["sigma_mat"]][4,6] , 365*3), rep(parms[["sigma_mat"]][4,7] , (365*3)+(7301-(365*3)*6))))
  }
...
parm_data <- data.frame(t(replicate(10000, runif(33, low_parm, high_parm))))
...
mono_func <- function(n, parms_frame, init, amr_ode, usage_fun, multi_int_fun, low_parm, high_parm, agg_func, thresh, approx_sigma) {
  ...
  parms_base = append(parms_base, parms["base_tax"])
  ...
  if(values[4] == 0 & values[5] == 0 & values[6] == 0 & values[7] == 0) {
    while(values[4] == 0 & values[5] == 0 & values[6] == 0 & values[7] == 0) {
      parms_base[c(1:33)] <- as.list(runif(33, low_parm, high_parm))
  ...
  for(i in 1:15){
    ...
    if(i >= 12 & i <= 15) {
      ban <- ban_wrapper(times = seq(0, 10300), init, parms, amr_ode, approx_sigma, ban = i-11)
      out <- ban[[1]]
      parms <- ban[[2]]
    }
    ...
    under_thresh <- sweep(out[out[,1] > parms[["t_n"]],][,19:22], 2, thresh)
...
comb_data_new <- data.frame(matrix(NA, nrow = nrow(comb_data), ncol = 60))

for(i in 1:nrow(comb_data)) {
  comb_data_new[i,] <- unlist(comb_data[i,1:60])
}

colnames(comb_data_new) <- colnames(comb_data)[1:60]

#Update the Parameter Set 
parm_data_comb_new <- parm_data_comb
parm_data_comb_new[1:nrow(comb_data),1:36] <- comb_data[,61:96]
...
for(i in 1:nrow(parm_data_comb_new)) {
  ...
  p_list <- append(p_list, parms["base_tax"])
  parm_list[[i]] <- p_list
}

#Save the output

saveRDS(parm_list, "/cluster/home/amorgan/Sens_Anal_Output/MDR_run_parms_four_tax.RDS")
saveRDS(comb_data_new, "/cluster/home/amorgan/Sens_Anal_Output/MDR_run_four_tax.RDS")
