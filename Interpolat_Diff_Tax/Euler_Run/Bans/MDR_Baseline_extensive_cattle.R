...
parms = list(lambda = 1/1095*(2), int_round = 1, 
             beta = 0.5, sigma1 = 0.25, sigma2 = 0.25, sigma3 = 0.25,
...
parm_data <- data.frame(t(replicate(10000, runif(20, low_parm, high_parm))))

colnames(parm_data) <- c("sigma1", "sigma2", "sigma3", 
...
mono_func <- function(n, parms_frame, init, amr_ode, usage_fun, multi_int_fun, low_parm, high_parm, agg_func, thresh, ode_wrapper, approx_sigma, ban_wrapper) {
  ...
  parms_base = append(parms_base, parms["beta"])
  parms_base = append(parms_base, parms["lambda"])
  ...
  if(values[4] == 0 & values[5] == 0 & values[6] == 0) {
    while(values[4] == 0 & values[5] == 0 & values[6] == 0) {
      parms_base[c(1:20)] <- as.list(runif(20, low_parm, high_parm))
...
#Save the output
saveRDS(parm_list, "/cluster/home/amorgan/Sens_Anal_Output/MDR_parms_cattle_extensive.RDS")
saveRDS(comb_data_new, "/cluster/home/amorgan/Sens_Anal_Output/MDR_cattle_extensive.RDS")
