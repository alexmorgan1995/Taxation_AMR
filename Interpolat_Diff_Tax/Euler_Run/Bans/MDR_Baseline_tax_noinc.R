...
ode_wrapper <- function(times, y, parms, func, approx_sigma) {
  ...
  if(parms[["int_round"]] > 0 ) {
    for(i in 1:parms[["int_round"]]) {
      ...
      sigma_mat[,(i+1):7][sigma_mat[,(i+1)] < 0.01] <- 0
...
parm_data <- data.frame(t(replicate(10000, runif(22, low_parm, high_parm))))
...
mono_func <- function(n, parms_frame, init, amr_ode, usage_fun, multi_int_fun, low_parm, high_parm, agg_func, thresh, ode_wrapper, approx_sigma, ban_wrapper) {
  ...
  if(values[4] == 0 & values[5] == 0 & values[6] == 0) {
    while(values[4] == 0 & values[5] == 0 & values[6] == 0) {
      parms_base[c(1:22)] <- as.list(runif(22, low_parm, high_parm))
  ...
  for(i in 1:7){
    ...
    if(i >= 5 & i <= 7) {
      ban <- ban_wrapper(times = seq(0, 10300), init, parms, amr_ode, approx_sigma, ban = i-4)
      out <- ban[[1]]
      parms <- ban[[2]]
    }
    ...
    under_thresh <- sweep(out[out[,1] > parms[["t_n"]],][,11:13], 2, thresh)
  ...
  names(output) <- c("flat_inf", "singleHR_inf", "singleMR_inf", "singleLR_inf", "banHR_inf", "banMR_inf", "banLR_inf", 
                     "flat_res", "singleHR_res", "singleMR_res", "singleLR_res", "banHR_res", "banMR_res", "banLR_res", 
                     "flat_shan", "singleHR_shan", "singleMR_shan", "singleLR_shan", "banHR_shan", "banMR_shan", "banLR_shan", 
                     "flat_avganti", "singleHR_avganti", "singleMR_avganti", "singleLR_avganti", "banHR_avganti", "banMR_avganti", "banLR_avganti", 
...
comb_data_new <- data.frame(matrix(NA, nrow = nrow(comb_data), ncol = 28))

for(i in 1:nrow(comb_data)) {
  comb_data_new[i,] <- unlist(comb_data[i,1:28])
}

colnames(comb_data_new) <- colnames(comb_data)[1:28]

#Save the output
saveRDS(comb_data_new, "/cluster/home/amorgan/Sens_Anal_Output/MDR_run_Base_tax_noinc.RDS")
