  ...
  if(int_gen > 1) {
    ...
    for(i in 2:int_gen) {
      ...
      if(values[1] == 0 & values[2] == 0 & values[3] == 0) {
        ...
        parms[["eff_tax"]][as.numeric(substr(low_char, 2, 2)), c((i):7)] <- as.numeric((parms[["base_tax"]]*(tail(run[low_char],1)/values_1rd[med_char_1rd])))
        parms[["eff_tax"]][as.numeric(substr(high_char, 2, 2)), c((i):7)] <- as.numeric((parms[["base_tax"]]*(tail(run[high_char],1)/values_1rd[med_char_1rd])))
        parms[["eff_tax"]][as.numeric(substr(med_char, 2, 2)), c((i):7)] <- as.numeric((parms[["base_tax"]]*(tail(run[med_char],1)/values_1rd[med_char_1rd])))
        parms[["eff_tax"]][parms[["eff_tax"]] < 0.00001] <- 0
      }
      parms[["int_round"]] <- i
    }
