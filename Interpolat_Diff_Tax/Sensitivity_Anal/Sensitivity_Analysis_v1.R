...
ode_function <- function(x, init, intervention, integral, ode_wrapper, approx_sigma, multi_int_fun, usage_fun) {
  return_vec <- vector()
  
  for(z in 1:nrow(x)) {
    
    #Create the Parameters
    parms = c(lambda = x[z,1], int_round = 1, 
    ...
    parms[["int_round"]] <- 1
    out_run_single <- ode_wrapper(y = init, func = amr_ode, times = seq(0, 10000), parms = parms, approx_sigma)
    out_single <- out_run_single[[1]]; parms_single <- out_run_single[[2]]
    
    #Differential Taxation 
    diff <- multi_int_fun(6, 365*3, parms, init, amr_ode, agg_func, ode_wrapper, approx_sigma)
    out_diff <- diff[[1]]
    parms_diff <- diff[[2]]
    
    #Integrals
    integral_single <- integral(out_run_single, 3000, 0.5)
    ...
    return_vec <- list(single_usage, diff_usage)
